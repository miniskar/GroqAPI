"""Build, compile and run C2C scatter collective op for fully connected A1.1 4-way topology."""
...
def create_data(input_tensor, result_tensors):
    ...
    oracles = {mt.name: input_data[i : i + 1] for i, mt in enumerate(result_tensors)}
...
def build_program(user_config, pgm_pkg, prog_name, speed):
    ...
    with pg_ctx:
        ...
        if (
            ...
            # Perform c2c scatter operation to devices 1, 2 and 3.
            ...
            # Perform c2c scatter operation to devices 1, 2 and 3.
            dst_devices = [g.device(1), g.device(2), g.device(3)]
            ...
            # Default Perform c2c scatter operation to devices 1, 2 and 3.
            # default assumes topology is FC2_A11_4_CHIP
            dst_devices = [g.device(1), g.device(2), g.device(3)]

        result_tensors = g.c2c_scatter(input_tensor, devices=dst_devices, time=10)
