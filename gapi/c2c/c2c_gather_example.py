"""Build, compile and run C2C gather collective op for fully connected A1.1 4-way topology."""
...
def create_data(input_tensors, result_tensor):
    ...
    oracle_data = np.concatenate(np.array(input_data), axis=-1)
...
def build_program(user_config, pgm_pkg, prog_name, speed):
    ...
    with pg_ctx:
        ...
        # Perform c2c gather operation on inner dim on device 0.
        result_tensor = g.c2c_gather(
            input_tensors, dest_device=g.device(0), dim=-1, time=10
        )
...
if __name__ == "__main__":
    ...
    prog_name = "c2c_gather"
    user_config = get_config_from_topo_str(args.topo_str)
    input_tensors, result_tensor = build_program(
